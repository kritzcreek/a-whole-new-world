...
(defvar motoko-ts-indent-rules
  `((motoko
     ...
     ((parent-is "array_exp") parent-bol motoko-ts-indent)
     ((parent-is "obj_typ") parent-bol motoko-ts-indent)
     ((parent-is "tup_typ") parent-bol motoko-ts-indent)
     ((parent-is "variant_typ") parent-bol motoko-ts-indent)
     ...
     ((parent-is "case") parent-bol motoko-ts-indent)
     ((parent-is "tup_pat") parent-bol motoko-ts-indent)
     ((parent-is "obj_pat") parent-bol motoko-ts-indent)
     )))
...
;; Do not commit this to the motoko repo as we don't have a "simple"
;; build command we could run here yet.
(require 'motoko-ts-flymake)
(add-hook 'motoko-ts-mode-hook 'motoko-ts-setup-flymake-backend)
(add-hook 'motoko-ts-mode-hook 'flymake-mode)
